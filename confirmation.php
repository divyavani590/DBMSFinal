...
//$result = $db_handle->runQuery("INSERT INTO 'order'('orderid') values (NULL)");
 $orderid = $db_handle->runQuery("SELECT * FROM ictya.order ORDER BY orderid DESC LIMIT 1");
foreach($_SESSION["cart_item"] as $k => $v) {
 /*    if($productByCode[0]["product_id"] == $k) {
    ...
    $uid = $_SESSION["email"];
    $db_handle->numRows("INSERT INTO ictya.orders('orderid', 'productid', 'quantity', 'user_id', 'order_status') VALUES (1, $prod_id, $qty, $uid , 'CONFIRMED' )");
} 
